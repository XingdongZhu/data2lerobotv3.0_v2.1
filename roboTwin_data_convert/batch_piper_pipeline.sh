...
# Pipeline: raw RoboTwin piper data -> HDF5 -> LeRobot (per job).
# Usage:
#   ./batch_piper_pipeline.sh              # run all jobs
#   ./batch_piper_pipeline.sh --dry-run    # print commands only
#   ./batch_piper_pipeline.sh --task shake_bottle

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
DATA_ROOT="${ROBOTWIN_DATA_ROOT:-/qinglong_datasets/qinglong/HuggingFace/RoboTwin2.0/dataset_unzip}"
HDF5_ROOT="${ROBOTWIN_HDF5_ROOT:-/qinglong_datasets/qinglong/HuggingFace/RoboTwin2.0/processed_data/sim_piper}"
LEROBOT_ROOT="${LEROBOT_OUTPUT_ROOT:-/qinglong_datasets/qinglong/lerobotv21/BAIHU_v3.1-p2_sim_robotwin2/sim_piper}"
ROBOT_TYPE="${ROBOTWIN_ROBOT_TYPE:-piper}"
LOG_DIR="${SCRIPT_DIR}/logs"
TIMESTAMP="$(date +%Y%m%d_%H%M%S)"
LOG_FILE="${LOG_DIR}/batch_piper_pipeline_${TIMESTAMP}.log"
...
Pipeline per job:
  ...
  2. hdf52lerobot_robotwin.sh   (HDF5 -> LeRobot under ${LEROBOT_ROOT}, robot_type=${ROBOT_TYPE})

Options:
  --dry-run         Print jobs without running conversion
  --task TASK_NAME  Process only one task (e.g. shake_bottle)
...
Environment:
  ...
  ROBOTWIN_ROBOT_TYPE    LeRobot robot type (default: ${ROBOT_TYPE})
EOF
      exit 0
      ;;
    *)
...
run_pipeline() {
  ...
  if $DRY_RUN; then
    ...
    echo "  [2/2] bash ${SCRIPT_DIR}/hdf52lerobot_robotwin.sh ${hdf5_dir} ${repo_id} ${LEROBOT_ROOT} ${ROBOT_TYPE}"
    return 0
  fi
  ...
  if ! bash "${SCRIPT_DIR}/hdf52lerobot_robotwin.sh" \
      "$hdf5_dir" \
      "$repo_id" \
      "$LEROBOT_ROOT" \
      "$ROBOT_TYPE" >>"$LOG_FILE" 2>&1; then
...
for task_dir in "${task_dirs[@]}"; do
  ...
  mapfile -t settings < <(find "$task_dir" -mindepth 1 -maxdepth 1 -type d -name 'piper*' | sort)
  if [[ ${#settings[@]} -eq 0 ]]; then
    echo "Skip ${task_name}: no piper* settings found"
    continue
  fi
