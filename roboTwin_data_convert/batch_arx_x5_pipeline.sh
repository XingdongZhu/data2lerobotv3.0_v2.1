...
# Pipeline: raw RoboTwin arx_x5 data -> HDF5 -> LeRobot (per job).
# Usage:
#   ./batch_arx_x5_pipeline.sh              # run all jobs
#   ./batch_arx_x5_pipeline.sh --dry-run    # print commands only
#   ./batch_arx_x5_pipeline.sh --task adjust_bottle

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
DATA_ROOT="${ROBOTWIN_DATA_ROOT:-/qinglong_datasets/qinglong/HuggingFace/RoboTwin2.0/dataset_unzip}"
HDF5_ROOT="${ROBOTWIN_HDF5_ROOT:-/qinglong_datasets/qinglong/HuggingFace/RoboTwin2.0/processed_data/sim_arx_x5}"
LEROBOT_ROOT="${LEROBOT_OUTPUT_ROOT:-/qinglong_datasets/qinglong/lerobotv21/BAIHU_v3.1-p2_sim_robotwin2/sim_arx_x5}"
ROBOT_TYPE="${ROBOTWIN_ROBOT_TYPE:-arx_x5}"
LOG_DIR="${SCRIPT_DIR}/logs"
TIMESTAMP="$(date +%Y%m%d_%H%M%S)"
LOG_FILE="${LOG_DIR}/batch_arx_x5_pipeline_${TIMESTAMP}.log"
...
for task_dir in "${task_dirs[@]}"; do
  ...
  mapfile -t settings < <(find "$task_dir" -mindepth 1 -maxdepth 1 -type d -name 'arx-x5*' | sort)
  if [[ ${#settings[@]} -eq 0 ]]; then
    echo "Skip ${task_name}: no arx-x5* settings found"
    continue
  fi
