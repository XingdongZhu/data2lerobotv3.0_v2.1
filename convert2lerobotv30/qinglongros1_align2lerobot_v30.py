#!/usr/bin/env python3
"""
青龙机器人 (QinLongROS1) 数据转换脚本 - 转换为 LeRobot 格式
...
数据结构:
    ...
    相机:
        - head: 640x480 RGB
        - hand_left: 640x480 RGB
        - hand_right: 640x480 RGB

Usage:
    python qinglongros1_align2lerobot_v30.py \
        --input ./raw_data \
        --output ./lerobot_dataset \
        --repo_id qinglong/task_name \
        --fps 30 --workers 4
"""
...
STATE_DIM = QINGLONG_CONFIG["arm_dim"] + QINGLONG_CONFIG["effector_dim"]  # 16
...
def main():
    parser = argparse.ArgumentParser(
        description="青龙机器人 (QinLongROS1) 数据转换为 LeRobot 格式"
    )
