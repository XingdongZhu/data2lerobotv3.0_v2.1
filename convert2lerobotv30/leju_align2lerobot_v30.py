#!/usr/bin/env python3
"""
乐聚夸父机器人数据转换脚本 - 转换为 LeRobot 格式

数据结构:
    State/Action: 30维 (某些数据集可能只有18维，缺少leg数据时用零填充)
        - arm: 14维 (left_arm 7 + right_arm 7)
        - effector: 2维 (left_claw + right_claw), 已归一化 [0,1]
        - head: 2维 (head_yaw + head_pitch)
        - leg: 12维 (left_leg 6 + right_leg 6) - 如果数据不存在则填充为0
    
    相机:
        - head: RGB
        - hand_left: RGB
        - hand_right: RGB
        - 统一 resize 到 480x640 (height x width)
...
LEJU_CONFIG = {
    "robot_type": "lejukuafu",
    "arm_dim": 14,           # 左臂7 + 右臂7
    "effector_dim": 2,       # 左夹爪 + 右夹爪
    "head_dim": 2,           # head_yaw + head_pitch
    "leg_dim": 12,           # 左腿6 + 右腿6
    "cameras": ["head", "hand_left", "hand_right"],
    "target_image_size": (640, 480),  # (width, height) - 统一resize目标尺寸：宽640、高480
}

# State/Action: arm(14) + effector(2) + head(2) + leg(12) = 30
STATE_DIM = LEJU_CONFIG["arm_dim"] + LEJU_CONFIG["effector_dim"] + LEJU_CONFIG["head_dim"] + LEJU_CONFIG["leg_dim"]
...
ACTION_NAMES = STATE_NAMES  # 动作名称与状态名称相同
...
def load_leju_h5(h5_path: Path) -> Dict[str, Any]:
    """
    从乐聚夸父 H5 文件加载数据
    
    注意：如果 H5 文件中没有 leg 数据，会自动用零填充到30维
    
    Returns:
        {
            'frames': int,
            'state': np.ndarray,  # (N, 30) = arm(14) + effector(2) + head(2) + leg(12)
            'action': np.ndarray, # (N, 30) = arm(14) + effector(2) + head(2) + leg(12)
            'images': {
    ...
    with h5py.File(h5_path, 'r') as f:
        ...
        # State: arm (14) + effector (2) + head (2) + leg (12) = 30
        # 注意：某些数据集可能没有 leg 数据，需要动态检测
        ...
        # 检查是否存在 leg 数据
        state_components = [arm_state, effector_state_norm, head_state]
        if 'joints/state/leg/position' in f:
            leg_state = f['joints/state/leg/position'][:]  # (N, 12)
            state_components.append(leg_state)
        else:
            # 如果没有 leg 数据，用零填充
            leg_state = np.zeros((arm_state.shape[0], 12), dtype=np.float32)
            state_components.append(leg_state)
            logger.warning(f"No leg state data found in {h5_path.name}, using zero padding")
        
        data['state'] = np.concatenate(state_components, axis=1).astype(np.float32)
        
        # Action: arm (14) + effector (2) + head (2) + leg (12) = 30
        ...
        # 检查是否存在 leg 数据
        action_components = [arm_action, effector_action, head_action]
        if 'joints/action/leg/position' in f:
            leg_action = f['joints/action/leg/position'][:]  # (N, 12)
            action_components.append(leg_action)
        else:
            # 如果没有 leg 数据，用零填充
            leg_action = np.zeros((arm_action.shape[0], 12), dtype=np.float32)
            action_components.append(leg_action)
        
        data['action'] = np.concatenate(action_components, axis=1).astype(np.float32)
        
        # 图像
        images = {}
        image_shapes = {}
        
        target_width, target_height = LEJU_CONFIG["target_image_size"]
        
        for cam_id in LEJU_CONFIG["cameras"]:
...
def convert_episode(
    ...
    try:
        ...
        dataset = LeRobotDataset.create(
            ...
            root=episode_dir,
            robot_type=LEJU_CONFIG["robot_type"],
            fps=fps,
            features=features,
            use_videos=True,
...
def convert_episode_wrapper(args: tuple) -> dict:
    ...
    try:
        episode_data = load_leju_h5(h5_path)
...
def main():
    parser = argparse.ArgumentParser(
        description="乐聚夸父机器人数据转换为 LeRobot 格式"
    )
