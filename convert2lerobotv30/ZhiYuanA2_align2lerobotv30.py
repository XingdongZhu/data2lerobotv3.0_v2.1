#!/usr/bin/env python3
"""
智元 A2 机器人数据转换脚本 - 转换为 LeRobot 格式

数据结构:
    State/Action:
        - arm: 14维 (left_arm 7 + right_arm 7)
        - fingers: 12维 (left hand 6 + right hand 6)
        - head: 2维 (head joint1 + head joint2)
        - 总计: 28维
    
    相机:
        - head_front: 640x480 RGB
        - chest_left: 640x480 RGB
        - chest_right: 640x480 RGB

Usage:
    python a2_align2_lerobot_v30.py \
        --input ./raw_data \
        --output ./lerobot_dataset \
        --repo_id zhiyuan_a2/task_name \
        --fps 30 --workers 4
"""
...
A2_CONFIG = {
    "robot_type": "ZhiYuanA2",
    "arm_dim": 14,           # 左臂7 + 右臂7
    "finger_dim": 12,        # 左手6 + 右手6
    "head_dim": 2,           # 头部2个关节
    "cameras": ["head_front", "chest_left", "chest_right"],
...
STATE_DIM = A2_CONFIG["arm_dim"] + A2_CONFIG["finger_dim"] + A2_CONFIG["head_dim"]  # 28
...
def load_a2_h5(h5_path: Path) -> Dict[str, Any]:
    """
    从智元 A2 机器人 H5 文件加载数据
    
    Returns:
        {
            'frames': int,
            'state': np.ndarray,  # (N, 28) = arm(14) + fingers(12) + head(2)
            'action': np.ndarray, # (N, 28) = arm(14) + fingers(12) + head(2)
            'images': {
    ...
    with h5py.File(h5_path, 'r') as f:
        ...
        # State: arm (14) + effector/fingers (12) + head (2)
        arm_state = f['joints/state/arm/position'][:]  # (N, 14)
        effector_state = f['joints/state/effector/position'][:]  # (N, 12)
        effector_state = np.clip(effector_state, 0.0000, 2000.0000)
        head_state = f['joints/state/head/position'][:]  # (N, 2)
        data['state'] = np.concatenate([arm_state, effector_state, head_state], axis=1).astype(np.float32)
        
        # Action: arm (14) + effector/fingers (12) + head (2)
        arm_action = f['joints/action/arm/position'][:]  # (N, 14)
        effector_action = f['joints/action/effector/position'][:]  # (N, 12)
        effector_action = np.clip(effector_action, 0.0000, 2000.0000)
        head_action = f['joints/action/head/position'][:]  # (N, 2)
        data['action'] = np.concatenate([arm_action, effector_action, head_action], axis=1).astype(np.float32)
        
        # 图像
        images = {}
        image_shapes = {}
        
        target_width, target_height = A2_CONFIG["target_image_size"]
        
        for cam_id in A2_CONFIG["cameras"]:
    ...
    try:
        ...
        features = {
            "observation.state": {
                "dtype": "float32",
                "shape": (STATE_DIM,),
                "names": {"motors": A2_CONFIG["motor_names"]},
                "fps": fps,
            },
            "action": {
                "dtype": "float32",
        ...
        dataset = LeRobotDataset.create(
            ...
            root=episode_dir,
            robot_type=A2_CONFIG["robot_type"],
            fps=fps,
            features=features,
            use_videos=True,
...
def convert_episode_wrapper(args: tuple) -> dict:
    ...
    try:
        episode_data = load_a2_h5(h5_path)
...
def main():
    parser = argparse.ArgumentParser(
        description="智元 A2 机器人数据转换为 LeRobot 格式"
    )
