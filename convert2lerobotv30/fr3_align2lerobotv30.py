#!/usr/bin/env python3
"""
FR3/FRANKA 数据转换脚本 - 转换为 LeRobot v3.0 格式

参考 Genie1_align2lerobotv30.py 的转换流程，并按 fr3_metadata.json 配置：
    State/Action:
        - arm: 14 维 (left arm 7 + right arm 7)
        - effector: 2 维 (left_gripper + right_gripper)
...
STATE_DIM = (
    FR3_CONFIG["arm_dim"]
    + FR3_CONFIG["effector_dim"]
    + FR3_CONFIG["end_position_dim"]
    + FR3_CONFIG["end_orientation_dim"]
)
...
def _read_end_pose(f: h5py.File, position_key: str, orientation_key: str) -> np.ndarray:
    """读取双臂末端位姿: left/right position + left/right quaternion."""
    end_position = f[position_key][:]  # (N, 2, 3)
    end_orientation = f[orientation_key][:]  # (N, 2, 4)
...
def load_fr3_h5(h5_path: Path) -> Dict[str, Any]:
    """
    从 FR3 H5 文件加载数据
    ...
    Returns:
        {
            'frames': int,
            'state': np.ndarray,  # (N, 30) = arm(14) + effector(2) + end_pose(14)
            'action': np.ndarray, # (N, 30) = arm(14) + effector(2) + end_pose(14)
            'images': {
    ...
    with h5py.File(h5_path, "r") as f:
        ...
        if "metadata.json" in f:
            raw_metadata = f["metadata.json"][()]
            metadata = json.loads(raw_metadata.decode() if isinstance(raw_metadata, bytes) else raw_metadata)
        data["task"] = metadata.get("task_name") or metadata.get("task") or "manipulation_task"

        timestamps = f["timestamp"][:]
        data["frames"] = len(timestamps)

        # State: arm (14) + effector (2) + end_pose (14)
        arm_state = f["joints/state/arm/position"][:]  # (N, 14)
        effector_state = f["joints/state/effector/position"][:]  # (N, 2)
        effector_state = np.clip(effector_state, 0.0, 1.0)
        end_state = _read_end_pose(
            f,
            "joints/state/end/position",
            "joints/state/end/orientation",
        )
        data["state"] = np.concatenate([arm_state, effector_state, end_state], axis=1).astype(np.float32)

        # Action: arm (14) + effector (2) + end_pose (14)
        arm_action = f["joints/action/arm/position"][:]  # (N, 14)
        effector_action = f["joints/action/effector/position"][:]  # (N, 2)
        effector_action = np.clip(effector_action, 0.0, 1.0)
        if "joints/action/end/position" in f and "joints/action/end/orientation" in f:
            end_action = _read_end_pose(
                f,
                "joints/action/end/position",
                "joints/action/end/orientation",
            )
        else:
            end_action = end_state
        data["action"] = np.concatenate([arm_action, effector_action, end_action], axis=1).astype(np.float32)

        images: Dict[str, List[np.ndarray]] = {}
        image_shapes: Dict[str, tuple] = {}
        target_width, target_height = FR3_CONFIG["target_image_size"]

        for cam_id in FR3_CONFIG["cameras"]:
            cam_key = f"cameras/{cam_id}/color/data"
            if cam_key not in f:
                logger.warning(f"Camera data not found: {cam_key}")
                continue
            ...
            for img_bytes in img_bytes_array:
                try:
                    ...
                    logger.warning(f"Failed to decode image from {cam_id}: {e}")

            if img_list:
                images[cam_id] = img_list[: data["frames"]]
    ...
    try:
        ...
        features = {
            "observation.state": {
                "dtype": "float32",
                "shape": (STATE_DIM,),
                "names": {"motors": FR3_CONFIG["motor_names"]},
                "fps": fps,
            },
            "action": {
                "dtype": "float32",
        ...
        dataset = LeRobotDataset.create(
            ...
            root=episode_dir,
            robot_type=FR3_CONFIG["robot_type"],
            fps=fps,
            features=features,
            use_videos=True,
...
def convert_episode_wrapper(args: tuple) -> dict:
    ...
    try:
        episode_data = load_fr3_h5(h5_path)
...
def main():
    parser = argparse.ArgumentParser(description="FR3/FRANKA 数据转换为 LeRobot v3.0 格式")
    parser.add_argument("--input", type=Path, required=True, help="数据目录 (包含 H5 文件)")
    parser.add_argument("--output", type=Path, required=True, help="输出目录")
    parser.add_argument("--repo_id", type=str, default=None, help="HuggingFace 仓库 ID")
    parser.add_argument(
        "--task",
        type=str,
        nargs="+",
    ...
    parser.add_argument("--fps", type=int, default=30, help="数据集帧率 (默认: 30)")
    parser.add_argument("--workers", type=int, default=8, help="并行工作进程数 (默认: 8)")
    parser.add_argument("--vcodec", type=str, default="libsvtav1", help="视频编码器 (默认: libsvtav1)")
    parser.add_argument("--crf", type=int, default=30, help="视频质量 CRF (默认: 30)")
