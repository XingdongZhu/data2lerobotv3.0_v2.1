#!/usr/bin/env python3
"""
zhengzhou_zhiyuan_G1 智元机器人 A2D 数据转换脚本 - 转换为 LeRobot 格式
...
A2D_CONFIG = {
    "robot_type": "zhengzhou_zhiyuan_G1",
    "arm_dim": 14,           # 左臂7 + 右臂7
...
def load_a2d_h5(h5_path: Path) -> Dict[str, Any]:
    ...
    with h5py.File(h5_path, 'r') as f:
        ...
        effector_state = np.clip(effector_state, 0.0, 120.0)
        data['state'] = np.concatenate([arm_state, effector_state], axis=1).astype(np.float32)
    ...
    try:
        ...
        features = {
            "observation.state": {
                "dtype": "float32",
                "shape": (STATE_DIM,),
                "names": {"motors": A2D_CONFIG["motor_names"]},
                "fps": fps,
            },
            "action": {
                "dtype": "float32",
