#!/usr/bin/env python3
"""
DWHEEL-TACTILE 数据转换脚本 - 转换为 LeRobot 格式
...
数据结构:
    State/Action:
        ...
        - waist: 2维 (pitch + lift)
        - robot velocity: 2维 (base_vx + base_vyaw)
        => 总计 20维

    相机:
        - head: RGB (当前只使用 color，不使用 depth/ir)
        - hand_left: RGB
        - hand_right: RGB
"""
...
STATE_DIM = (
    DWHEEL_CONFIG["arm_dim"]
    + DWHEEL_CONFIG["effector_dim"]
    + DWHEEL_CONFIG["waist_dim"]
    + DWHEEL_CONFIG["robot_dim"]
)  # 20
...
def encode_video(images: List[np.ndarray], video_path: Path, fps: int, vcodec: str = "libsvtav1", crf: int = 30) -> None:
    """使用最佳可用方法编码视频"""
    if USE_PYAV:
...
def load_dwheel_h5(h5_path: Path) -> Dict[str, Any]:
    """
    从 DWHEEL H5 文件加载数据
    ...
    with h5py.File(h5_path, 'r') as f:
        ...
        # State: arm(14) + effector(2) + waist(2) + robot(2)
        arm_state = f['joints/state/arm/position'][:]              # (N, 14)
        effector_state = f['joints/state/effector/position'][:]    # (N, 2)
        effector_state = np.clip(effector_state, 0.0, 0.09)
        waist_state = f['joints/state/waist/position'][:]          # (N, 2)
        robot_state = f['joints/state/robot/velocity'][:]          # (N, 2)
        data['state'] = np.concatenate([arm_state, effector_state, waist_state, robot_state], axis=1).astype(np.float32)

        # Action: arm(14) + effector(2) + waist(2) + robot(2)
        arm_action = f['joints/action/arm/position'][:]            # (N, 14)
        effector_action = f['joints/action/effector/position'][:]  # (N, 2)
        effector_action = np.clip(effector_action, 0.0, 0.09)
        waist_action = f['joints/action/waist/position'][:]        # (N, 2)
        robot_action = f['joints/action/robot/velocity'][:]        # (N, 2)
        data['action'] = np.concatenate([arm_action, effector_action, waist_action, robot_action], axis=1).astype(np.float32)

        # 图像（当前只读取 RGB color）
        images: Dict[str, List[np.ndarray]] = {}
        image_shapes: Dict[str, tuple] = {}
        target_width, target_height = DWHEEL_CONFIG["target_image_size"]

        for cam_id in DWHEEL_CONFIG["cameras"]:
            cam_key = f'cameras/{cam_id}/color/data'
            if cam_key not in f:
                logger.warning(f"Camera key not found: {cam_key}, skipping")
                continue
    ...
    try:
        ...
        features = {
            "observation.state": {
                "dtype": "float32",
                "shape": (STATE_DIM,),
                "names": {"motors": DWHEEL_CONFIG["motor_names"]},
                "fps": fps,
            },
            "action": {
                "dtype": "float32",
        ...
        dataset = LeRobotDataset.create(
            ...
            root=episode_dir,
            robot_type=DWHEEL_CONFIG["robot_type"],
            fps=fps,
            features=features,
            use_videos=True,
        ...
        episode_buffer["task_index"] = np.array([dataset.meta.get_task_index(t) for t in tasks_list])
...
def convert_episode_wrapper(args: tuple) -> dict:
    ...
    try:
        episode_data = load_dwheel_h5(h5_path)
...
def main():
    parser = argparse.ArgumentParser(description="DWHEEL-TACTILE 数据转换为 LeRobot 格式")
    ...
    parser.add_argument("--task", type=str, nargs="+", default=["manipulation_task"], help="任务描述 (多个词会自动拼接)")
    ...
    print(f"Using {args.workers} workers, PyAV: {USE_PYAV}")
    print(f"State/Action dim: {STATE_DIM}")
    print(f"Cameras: {DWHEEL_CONFIG['cameras']}")
    ...
    for i, path in enumerate(episodes):
        tasks.append((path, separate_dir, args.repo_id, i, args.fps, args.vcodec, args.crf, args.task))
