#!/usr/bin/env python3
"""
天机 (TIANJI) 机器人数据转换脚本 - 转换为 LeRobot v3.0 格式

设备信息:
    manufacturer: 人形机器人（上海）有限公司
    model: TIANJI V1

数据结构 (拼接顺序: 左臂+左夹爪+右臂+右夹爪):
    State (16维):
        left_arm(7) + left_gripper(1) + right_arm(7) + right_gripper(1)
        effector clip 到 [0, 100]
    Action (16维):
        left_arm(7) + left_gripper(1) + right_arm(7) + right_gripper(1)
        effector clip 到 [0, 100]
...
用法:
    ...
    python tianji_align2lerobot_v30.py \\
        --input  /path/to/task_dir \\
...
TIANJI_CONFIG = {
    "robot_type": "TIANJI",
    "arm_dim": 14,           # l_j1~l_j7 + r_j1~r_j7
...
STATE_DIM = TIANJI_CONFIG["arm_dim"] + TIANJI_CONFIG["effector_dim"]  # 16
...
def load_tianji_h5(h5_path: Path) -> Dict[str, Any]:
    """
    从 TIANJI H5 文件加载数据
    ...
    with h5py.File(h5_path, 'r') as f:
        ...
        effector_state = np.clip(effector_state, 0.0, 100.0)
        data['state'] = np.concatenate([
            arm_state[:, :7], effector_state[:, :1],
            arm_state[:, 7:], effector_state[:, 1:],
        ], axis=1).astype(np.float32)

        # --- Action: left_arm(7) + left_gripper(1) + right_arm(7) + right_gripper(1) ---
        arm_action = f['joints/action/arm/position'][:]          # (N, 14)
        effector_action = f['joints/action/effector/position'][:] # (N, 2)
        effector_action = np.clip(effector_action, 0.0, 100.0)
        data['action'] = np.concatenate([
            arm_action[:, :7], effector_action[:, :1],
            arm_action[:, 7:], effector_action[:, 1:],
        ], axis=1).astype(np.float32)
        ...
        images = {}
        image_shapes = {}
        target_width, target_height = TIANJI_CONFIG["target_image_size"]

        for cam_id in TIANJI_CONFIG["cameras"]:
    ...
    try:
        ...
        features = {
            "observation.state": {
                "dtype": "float32",
                "shape": (STATE_DIM,),
                "names": {"motors": TIANJI_CONFIG["motor_names"]},
                "fps": fps,
            },
            "action": {
                "dtype": "float32",
        ...
        dataset = LeRobotDataset.create(
            ...
            root=episode_dir,
            robot_type=TIANJI_CONFIG["robot_type"],
            fps=fps,
            features=features,
            use_videos=True,
...
def convert_episode_wrapper(args: tuple) -> dict:
    ...
    try:
        episode_data = load_tianji_h5(h5_path)
...
def main():
    parser = argparse.ArgumentParser(
        description="天机 (TIANJI) 机器人数据转换为 LeRobot v3.0 格式"
    )
