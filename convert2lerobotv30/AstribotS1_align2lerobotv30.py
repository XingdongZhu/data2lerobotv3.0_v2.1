...
S1 机器人数据结构 (基于 metadata.json ver=2.1.0):
    ...
    备注：
        - h5 中还有 chassis(3) / state/robot(N,5,3 与 N,5,4) IMU/力矩数据，
...
def load_astribot_s1_h5(h5_path: Path, cameras: List[str]) -> Dict[str, Any]:
    ...
    Returns:
        {
            'frames': int,
            'state':  np.ndarray (N, 22),
            'action': np.ndarray (N, 22),
    ...
    with h5py.File(h5_path, 'r') as f:
        ...
        data['state'] = np.concatenate([s_arm, s_effector, s_head, s_torso], axis=1).astype(np.float32)
        ...
        data['action'] = np.concatenate([a_arm, a_effector, a_head, a_torso], axis=1).astype(np.float32)
...
def main():
    ...
    print(f"State/Action dim: {STATE_DIM} (arm14 + effector2 + head2 + torso4)")
