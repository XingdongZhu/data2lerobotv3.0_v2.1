...
def convert_tasks(root: Path, new_root: Path) -> None:
    logging.debug("Converting tasks parquet to legacy JSONL")
...
def convert_info(
    ...
    info = load_info(root)
    logging.debug("Converting info.json metadata to v2.1 schema")
...
def convert_data(root: Path, new_root: Path, episode_records: list[dict[str, Any]]) -> None:
    logging.debug("Converting consolidated parquet files back to per-episode files")
    grouped = _group_episodes_by_data_file(episode_records)
    
    # 检查环境变量决定是否显示进度条
    show_progress = os.environ.get('TQDM_DISABLE', '1') == '0'

    for (chunk_idx, file_idx), records in tqdm.tqdm(grouped.items(), desc="convert data files", disable=not show_progress):
...
def convert_videos(root: Path, new_root: Path, episode_records: list[dict[str, Any]], video_keys: list[str]) -> None:
    if len(video_keys) == 0:
        logging.debug("No video features detected; skipping video conversion")
        return

    logging.debug("Converting concatenated MP4 files back to per-episode videos")

    for video_key in video_keys:
        grouped = _group_episodes_by_video_file(episode_records, video_key)
        if len(grouped) == 0:
            logging.debug("No video metadata found for key '%s'; skipping", video_key)
            continue

        # 检查环境变量决定是否显示进度条
        show_progress = os.environ.get('TQDM_DISABLE', '1') == '0'
        for (chunk_idx, file_idx), records in tqdm.tqdm(grouped.items(), desc=f"convert videos ({video_key})", disable=not show_progress):
...
def convert_episodes_metadata(new_root: Path, episode_records: list[dict[str, Any]]) -> None:
    logging.debug("Reconstructing legacy episodes and episodes_stats JSONL files")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path where the v2.1 dataset will be saved. If not provided, it will create a sibling directory with _v2.1 suffix.",
    )
